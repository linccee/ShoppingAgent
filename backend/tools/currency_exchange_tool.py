...
def currency_exchange(base_code: str, target_code: str, amount: str) -> str:
    ...
    """
    tools_logger.info(f"[TOOLS] currency_exchange called: {amount} {base_code} -> {target_code}")
    url = f"https://v6.exchangerate-api.com/v6/{Config.EXCHANGE_RATE_API_KEY}/pair/{base_code}/{target_code}/{amount}"
    try:
        response = requests.get(url, timeout=10)
        data = response.json()
    except Exception as e:
        tools_logger.error(f"[TOOLS] currency_exchange network error: {e}")
        return json.dumps({"success": False, "error": f"网络请求失败: {e}"}, ensure_ascii=False)

    if data.get("result") != "success":
        tools_logger.warning(f"[TOOLS] currency_exchange API error: {data.get('error-type', 'unknown')}")
        return json.dumps({
            "success": False,
            "error": f"API 请求失败，错误类型: {data.get('error-type', '未知错误')}"
        }, ensure_ascii=False)

    tools_logger.info(f"[TOOLS] currency_exchange success: {amount} {base_code} = {data.get('conversion_result')} {target_code}")
