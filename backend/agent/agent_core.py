...
from backend.agent.compressed_checkpointer import CompressedCheckpointer
from backend.utils.db import client
...
def create_shopping_agent():
    ...
    # 2. 注册工具列表
    from backend.tools.currency_exchange_tool import currency_exchange
    from backend.tools.price_tool import prices
    from backend.tools.review_tool import analyze_reviews
    from backend.tools.search_tool import search_products
    from backend.tools.tavily_tool import tavily_search, tavily_extract
...
def stream_agent(
    ...
    cb = _QueueCallback(q, resolved_stop_event)
